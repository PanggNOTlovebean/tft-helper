...
idldsl.define_winrt_com_method(ID3D11Device, 'CreateTexture2D', ctypes.POINTER(D3D11_TEXTURE2D_DESC), ctypes.c_void_p,
                               retval=ID3D11Texture2D, vtbl=5)
...
idldsl.define_winrt_com_method(ID3D11DeviceContext, 'Map', IUnknown, ctypes.c_uint, ctypes.c_int, ctypes.c_uint,
                               retval=D3D11_MAPPED_SUBRESOURCE, vtbl=14)
...
D3D11CreateDevice.argtypes = (
    IUnknown, ctypes.c_int, ctypes.c_void_p, ctypes.wintypes.UINT, ctypes.c_void_p, ctypes.c_uint, ctypes.c_uint,
    ID3D11Device, ctypes.c_void_p, ID3D11DeviceContext)
